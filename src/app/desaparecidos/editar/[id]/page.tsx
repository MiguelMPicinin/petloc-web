...
export default function EditarDesaparecidoPage() {
  const { user } = useAuth();
  ...
  const loadDesaparecido = async () => {
    try {
      const docRef = doc(db, 'desaparecidos', id);
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        const data = docSnap.data() as Desaparecido;
        ...
        if (data.imagemBase64) {
          setImagemPreview(`data:image/jpeg;base64,${data.imagemBase64}`);
        }
      } else {
        setError('Registro não encontrado');
      }
    } catch (error: any) {
      setError('Erro ao carregar registro: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'desaparecidos', id), updateData);
      router.push('/desaparecidos');
    } catch (error: any) {
      ...
      <header className="header">
        <div className="header-content">
          <div className="flex items-center space-x-3">
            ...
            <h1 className="text-xl font-bold">Editar Registro</h1>
          </div>
        </div>
      </header>
          ...
          <div className="card space-y-6">
            ...
            <div className="flex items-center">
              <input
                ...
                id="encontrado"
                checked={encontrado}
                onChange={(e) => setEncontrado(e.target.checked)}
                className="w-4 h-4 text-primary-color border-gray-300 rounded focus:ring-primary-color"
              />
              <label htmlFor="encontrado" className="ml-2 text-sm text-on-surface">
